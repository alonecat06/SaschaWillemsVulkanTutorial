	...
	*/
	...
	void drawNode(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout, VulkanglTFModel::Node* node)
	{
		if (node->mesh.primitives.size() > 0) {
			...
			nodeMatrix = glm::scale(nodeMatrix, glm::vec3(0.05f));
			...
			vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof(glm::mat4)
			...
			for (VulkanglTFModel::Primitive& primitive : node->mesh.primitives) {
				if (primitive.indexCount > 0) {
					...
					vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout
						, 1 , 1, &images[texture.imageIndex].descriptorSet
						, 0, nullptr);

					for (int i = 0; i < furLayerNum; i++)
					{
						float layerRatio = static_cast<float>(i) / static_cast<float>(furLayerNum);
						vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT
							, sizeof(glm::mat4) + sizeof(float), sizeof(float), &layerRatio);
...
public:
	...
	void loadAssets()
	{
		loadglTFFile(getAssetPath() + "models/bunny/scene.gltf");
	}
	...
	void preparePipelines()
	{
		...
		VkPushConstantRange pushConstantRange = vks::initializers::pushConstantRange(VK_SHADER_STAGE_VERTEX_BIT, sizeof(float) + sizeof(glm::mat4), 0);
